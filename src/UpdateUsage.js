...
const UpdateUsage = ({ token }) => {
  ...
  useEffect(() => {
    ...
    const ws = new WebSocket(`ws://localhost:9090/updateUsage?token=${token}`);
  ...
  const sendUsageData = (newUsageData) => {
    if (socket && socket.readyState === WebSocket.OPEN) {
      socket.send(JSON.stringify(newUsageData));
      setUsageData(newUsageData);
    } else {
  ...
  const handleManualUpdate = (e) => {
    ...
    sendUsageData(newUsageData);
    setFormData({ waterUsage: "", electricityUsage: "" });
  };
    ...
    <div className="container mt-4">
      <div className="card">
        ...
        <div className="card-body">
          ...
          <div className="mb-3">
            <label style={{ marginRight: "20px" }}>
              <input type="radio" value="automatic" checked={isAutomatic} onChange={() => setIsAutomatic(true)} />
              Simulate the usage
            </label>
            <label>
              <input type="radio" value="manual" checked={!isAutomatic} onChange={() => setIsAutomatic(false)} />
              Manually Update Your Usage Data
            </label>
          </div>
          {isAutomatic ? (
            ...
            <form onSubmit={handleManualUpdate}>
              <div className="mb-3">
                <label htmlFor="waterUsage" className="form-label">Water Usage (liters):</label>
                <input type="number" className="form-control" id="waterUsage" value={formData.waterUsage} onChange={(e) => setFormData({ ...formData, waterUsage: e.target.value })} required />
              </div>
              <div className="mb-3">
                <label htmlFor="electricityUsage" className="form-label">Electricity Usage (W):</label>
                <input type="number" className="form-control" id="electricityUsage" value={formData.electricityUsage} onChange={(e) => setFormData({ ...formData, electricityUsage: e.target.value })} required />
              </div>
              <button type="submit" className="btn btn-primary">Update Manually</button>
            </form>
          )}
